...
def print_text():
    text = tkinter.Label(frame_2, text=entry_1.get())  # entry.get(): フォームに入力した文字列を取得
    text.pack()

    entry_1.delete(0, END)  # entry widgetsの中の0文字目から最後までをdelete


def count(number):
    global value  # 関数外でもvalueは使うためglobal変数としている
...
frame_1.grid_propagate(False)  # エントリーのサイズに依存してフレームの大きさが変化する、True, False
...
button_2 = tkinter.Button(
    frame_1, text="カウント", command=lambda: count(value)
)  # lambda: 関数
